...
export class ConfigService {
  ...
  private constructor() {
    ...
    logManager.info("ConfigService init success");
  }
  private _setupIpcEvents() {
    ...
    ipcMain.on(IPC_EVENTS.SET_CONFIG, (_, key, value) => this.set(key, value));
  ...
  private _notifyListeners(): void {
    BrowserWindow.getAllWindows().forEach((window) =>
      window.webContents.send(IPC_EVENTS.CONFIG_UPDATED, this._config)
    );
  ...
  public resetToDefault(): void {
    ...
    logManager.info("Reset config to default");
    this._saveConfig();
  }
