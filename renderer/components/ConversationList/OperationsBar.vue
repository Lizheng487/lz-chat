...
watch([() => ctx?.checkedIds.value.length, () => conversations.value.length], ([checkIdsSize, conversationsSize]) => {
  ...
  <div @click.stop>
    ...
    <div class="flex justify-between items-center pt-1">
      <n-checkbox v-model:checked="isAllSelected" @update:checked="handleSelectChange">{{
        $t('main.conversation.operations.selectAll') }}</n-checkbox>
      <n-button quaternary @click="emits('cancel')">{{ $t('main.conversation.operations.cancel') }}</n-button>
    </div>
    <div class="flex items-center py-4">
      <n-button class="flex-1" style="margin-right: 2px;" @click="emits('op', CONVERSATION_ITEM_MENU_IDS.PIN)">{{
        $t('main.conversation.operations.pin') }}</n-button>
      <n-button class="flex-1" @click="emits('op', CONVERSATION_ITEM_MENU_IDS.DEL)">{{
        $t('main.conversation.operations.del') }}</n-button>
    </div>
  </div>
</template>